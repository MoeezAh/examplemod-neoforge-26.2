...
- **Kaupenger, level 1** — 12 Emeralds for a Metal Detector, or 12 Emeralds for Raw Azurite. Each trade has 12 uses and awards 6 villager experience.
- **Kaupenger, level 2** — 10 Emeralds for a Metal Detector, or 10 Azurite for a Magic Block. Each trade has 12 uses and awards 6 villager experience.

Trade definitions are generated under `src/generated/resources/data/examplemod/villager_trade/` and assigned to the corresponding villager level tags. The Kaupenger profession uses the Magic Block as its point of interest, and its trade sets are generated under `src/generated/resources/data/examplemod/trade_set/`.
...
The mod adds a small advancement tree generated by `ModAdvancements`:

- **Example Mod Advancements** is the root advancement. It is completed when the player obtains Azurite and uses the adventure background without showing a toast or chat announcement.
- **Plant a custom plant** is unlocked from the root and is completed when the player plants a Goji Berry Bush, Rice Crop, or Onion Crop. It uses OR requirements, so planting any one of the three crops is sufficient.
- **Detected Valuables** is unlocked from the root and is completed when the player uses the Metal Detector on a block while the detector position can see the sky.

Advancement titles and descriptions are localized in `en_us.json`, while the advancement JSON is generated under `src/generated/resources/data/` and registered through the data-generation pipeline.
